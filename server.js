...
// ส่วนนี้อยู่ใน server.js ในฟังก์ชัน app.post('/create-appointment')
app.post('/create-appointment', async (req, res) => {
    const { user_id, program_name, appointment_date } = req.body;

    // กำหนดค่า max_slots สำหรับแต่ละโปรแกรมตรวจ
    const programSlots = {
        'ตรวจสุขภาพ': 10,  // ตรวจสุขภาพจำกัด 10 คิวต่อวัน
        'HIV': 5,           // HIV จำกัด 5 คิวต่อวัน
        'เลิกบุหรี่': 3     // เลิกบุหรี่จำกัด 3 คิวต่อวัน
    };

    try {
        const formattedDate = moment(appointment_date, 'DD/MM/YYYY').format('YYYY-MM-DD');

        // ตรวจสอบจำนวนการจองในวันนั้น
        const { count } = await db.one(
            'SELECT COUNT(*) FROM appointments WHERE program_name = $1 AND appointment_date = $2',
            [program_name, formattedDate]
        );

        // ดึงค่า max_slots สำหรับโปรแกรมตรวจนั้นๆ
        const maxSlots = programSlots[program_name];

        // ตรวจสอบว่าจำนวนการจองเกินหรือไม่
        if (parseInt(count) >= maxSlots) {
            return res.status(400).json({ message: 'จำนวนการจองเต็มแล้วในวันนี้' });
        }

        // สร้างการนัดหมายใหม่
        const result = await db.one(`
            INSERT INTO appointments (user_id, program_name, result_program, appointment_date, max_slots) 
            VALUES ($1, $2, NULL, $3, $4) 
            RETURNING id`, [user_id, program_name, formattedDate, maxSlots]);
...
app.get('/appointments/available-slots/:program_name', async (req, res) => {
    ...
    try {
        ...
        const maxSlots = await db.one(
            'SELECT max_slots FROM appointments WHERE program_name = $1 LIMIT 1',
            [programName]
        );

        const availableSlots = maxSlots.max_slots - parseInt(count);
...
app.get('/appointments/full-dates/:program_name', async (req, res) => {
    const programName = req.params.program_name;

    try {
        const fullDates = await db.any(`
...
// Route ที่ดึงข้อมูลจำนวนคิวที่เหลือในวันนั้น
app.get('/appointments/available-slots/:program_name/:date', async (req, res) => {
    const { program_name, date } = req.params;
